...
print("--- 🏁 BOTAS STARTUOJA (Automatinis Modelio Parinkimas) ---")
...
# --- DINAMINIS MODELIO RADIMAS ---
def get_working_model():
    """Randa pirmą veikiantį Gemini 1.5 Flash modelį tavo paskyroje."""
    url = f"https://generativelanguage.googleapis.com/v1beta/models?key={GEMINI_KEY}"
    try:
        res = requests.get(url).json()
        if 'models' not in res:
            print(f"🚨 Klaida pasiekiant modelių sąrašą: {res}")
            return "gemini-1.5-flash" # Default fallback
        
        # Ieškome flash modelio
        for m in res['models']:
            name = m['name'].split('/')[-1] # Nuimame 'models/' prefiksą
            if "gemini-1.5-flash" in name:
                print(f"✅ Rastas modelis: {name}")
                return name
        return "gemini-1.5-flash"
    except Exception as e:
        print(f"⚠️ Nepavyko gauti modelių sąrašo, naudojamas standartas: {e}")
        return "gemini-1.5-flash"

MODEL_NAME = get_working_model()
GEMINI_URL = f"https://generativelanguage.googleapis.com/v1beta/models/{MODEL_NAME}:generateContent?key={GEMINI_KEY}"

# --- WP KATEGORIJOS IR NUSTATYMAI ---
...
def run_wealth_bot(politician_name):
    ...
    prompt = (
        ...
        f"1. WEALTH: 2026 estimate (add 8% annual growth if data is old). \n"
